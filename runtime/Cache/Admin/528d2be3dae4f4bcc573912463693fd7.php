...
<div class="table-toolbar">
    <a class="btn btn-primary" href="<?php echo U('model/index','id='.$_GET['modelid']);?>"><i class="fa fa-reply"></i>返回模型管理</a>
    <a class="btn btn-primary" href="<?php echo U('Field/add',array(modelid=>$_GET[modelid]));?>">添加</a>
    <a class="btn btn-primary" onclick="myform.action='<?php echo U('field/listorder');?>';$('#myform').submit();" >排序</a>
</div>
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
            <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                <td><input name='listorders[<?php echo ($vo["id"]); ?>]' type='text' size='3' value='<?php echo ($vo["listorder"]); ?>' class='input-text-c'></td>
                <td><?php echo ($vo['field']); ?></td>
                <td><?php echo ($vo['name']); ?></td>
                <td><?php echo ($vo['type']); ?></td>
                <td><?php if(in_array($vo['field'],$sysfield)): ?><font color="green">√</font><?php else: ?><font color="red"">×</font><?php endif; ?></td>
                <td><?php if($vo['required']==1): ?><font color="green">√</font><?php else: ?><font color="red"">×</font><?php endif; ?></td>
                <td>
                <a class="btn btn-xs btn-primary" href="<?php echo U('field/edit',array(modelid=>$vo['modelid'],id=>$vo['id']));?>" >修改</a>
                <?php if($vo[status]==1): if(in_array($vo['field'],$nodostatus)): ?><a class="btn btn-xs btn-default" disabled>禁用</a>
                    <?php else: ?>
                        <a class="btn btn-xs btn-warning" href="<?php echo U('field/status',array(id=>$vo['id'],status=>0));?>">禁用</a><?php endif; ?>
                <?php else: ?>
                    <a href="<?php echo U('field/status',array(id=>$vo['id'],status=>1));?>" class="btn btn-xs btn-info">启用</a><?php endif; ?>
               <?php if(in_array($vo['field'],$sysfield)): ?><a class="btn btn-xs btn-default" disabled>删除</a>
                <?php else: ?>
                <a class="btn btn-xs btn-danger" href="javascript:confirm_delete('<?php echo U('field/delete',array(id=>$vo['id']));?>')">删除</a><?php endif; ?>
            </td>

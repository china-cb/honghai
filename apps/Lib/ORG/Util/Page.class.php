...
class Page {
    ...
    public $coolPages   ;
    // 分页显示定制
    public $config  =	array('header'=>'条','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'末尾','theme'=>'%totalRow%%header% %nowPage%/%totalPage%页 %first% %prePage% %upPage% %linkPage% %downPage% %nextPage% %end%');
    ...
    public function __construct($totalRows,$listRows='',$parameter='') {
        ...
        {
        	$this->nowPage  = $_REQUEST['pageNum'];/*兼容JUI*/
        }
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
                    $linkPage.="<a class='number' href='".$url."&".$p."=$page'>".$page."</a>";
                }else{
                    break;
                }
            }else{
